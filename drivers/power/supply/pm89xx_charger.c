// from https://github.com/flto/linux/blob/msm8930/drivers/power/supply/qcom_pm8921.c
/* SPDX-License-Identifier: GPL-2.0 */
/* hacked up qcom_smbb.c based off downstream pm89xx-charger.c
				   ...
				   enum smbb_attr which, unsigned int val)
{
	...
	if (prop->safe_reg) {
		rc = regmap_read(chg->regmap, chg->addr + prop->safe_reg,
				 &wval);
		if (rc) {
			dev_err(chg->dev,
	...
	out = prop->hw_fn(wval);
	if (out != val) {
		dev_warn(chg->dev, "%s inaccurate, rounded to %u\n", prop->name,
			 out);
	}
